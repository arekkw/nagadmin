...
export default Ember.View.extend({
    _successMsg: function(){
        ...
        if(msg && msg.length >0){
            Ember.$('.user-list .alert-success').removeClass('hide').show( 300 ).delay( 2000 ).fadeOut( 400, function(){
    ...
    _errorMsg: function(){
        ...
        var msg = this.get('controller.successMsg');
        if(msg && msg.length >0){
            Ember.$('.user-list .alert-error').removeClass('hide').show( 300 ).delay( 2000 ).fadeOut( 400, function(){
